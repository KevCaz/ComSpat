...
**Table 1.** Properties of the simulation and randomized grids and real transect data sets included within the package. The table shows the name, the type, dimensions (Dim), number of species (NSP), simulated drivers of the spatial patterns and the source of the data sets provided by the package.
...
```{r, fig.width = 7.1, fig.cap="**Fig. S7**. Frequency distribution of the species and plant functional types (PFT)  within the transect data sets."}
# View the species frequency of occurrence
...
```{r, fig.cap="**Fig. S8**. Example of spatial scaling using exponentially increaseing of in secondary sampling unit sizes."}
...
Here, using visual aids, we demonstrate NRC and CD's calculation at two scaling steps (spatial-scale 1 and -scale 2) using example grid data (**Figure S9**). The mathematical formula follows Juhász-Nagy (1967, 1976, 1984b, 1984a, 1993) summarized here.
...
![**Fig. S10**. Illustration and calculation of the number of realized species combinations (NRC) and compositional diversity (CD) using an example data set sampled as a grid. The example focuses on illustrating the secondary sampling procedure a spatial scale of 2 × 2 grains.](S1_Box_2_Landscape.jpg){width="95%"}
...
As the level of complexity decreases (i.e. the number of assembly mechanisms involved in forming the simulated data) from left to right, we can observe a gradual increase in the amplitude of the observed peaks correlated to an increase in the area with which they are observed.
...
axis(1, at=round(c(0.01,0.1,1,10,100),2), labels=c(0.01, 0.1,1,10,100), cex.axis=1, tck = -0.025)
mtext("Length of sampling units (m)", side =1, line = 4, font =2)
text(x=0.0125,y=7.5,"C", font = 2, cex = 1)
...
axis(1, at=round(c(0.01,seq(0.02,0.09,0.01),0.1,1,10,100),2), labels=FALSE, cex.axis=0.75, tck = -0.015)
axis(1, at=round(c(0.01,0.1,1,10,100),2), labels=c(0.01, 0.1,1,10,100), cex.axis=1, tck = -0.025)
text(x=0.0125,y=4.5,"D", font = 2, cex = 1)
